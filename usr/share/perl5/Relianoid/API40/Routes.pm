...
if ($q->path_info =~ qr{^/certificates/letsencryptz}) {
    require Relianoid::API40::LetsencryptZ;
...
if ($q->path_info =~ qr{^/certificates}) {
    require Relianoid::API40::Certificate;
...
if ($q->path_info =~ qr{^/farms/$farm_re(?:/services/$service_re)?/backends}) {
    require Relianoid::API40::Farm::Backend;
...
if ($q->path_info =~ qr{^/farms/$farm_re/services}) {
    require Relianoid::API40::Farm::Service;
...
if ($q->path_info =~ qr{^/farms}) {
    if ($ENV{REQUEST_METHOD} eq 'GET') {
        require Relianoid::API40::Farm::Get;
    ...
    if ($ENV{REQUEST_METHOD} eq 'POST') {
        require Relianoid::API40::Farm::Post;
        ##### /farms
        POST qr{^/farms$} => \&new_farm;
    }

    if ($ENV{REQUEST_METHOD} eq 'PUT') {
        require Relianoid::API40::Farm::Put;
    ...
    if ($ENV{REQUEST_METHOD} eq 'DELETE') {
        require Relianoid::API40::Farm::Delete;
...
if ($q->path_info =~ qr{^/interfaces/nic}) {
    require Relianoid::API40::Interface::NIC;
...
if ($q->path_info =~ qr{^/interfaces/vlan}) {
    require Relianoid::API40::Interface::VLAN;
...
if ($q->path_info =~ qr{^/interfaces/virtual}) {
    require Relianoid::API40::Interface::Virtual;
...
if ($q->path_info =~ qr{^/interfaces$}) {
    require Relianoid::API40::Interface::Generic;
...
if ($q->path_info =~ qr{^/system/dns}) {
    require Relianoid::API40::System::Service::DNS;
...
if ($q->path_info =~ qr{^/system/snmp}) {
    require Relianoid::API40::System::Service::SNMP;
...
if ($q->path_info =~ qr{^/system/ntp}) {
    require Relianoid::API40::System::Service::NTP;
...
if ($q->path_info =~ qr{^/system/users}) {
    require Relianoid::API40::System::User;
...
if ($q->path_info =~ qr{^/system/log}) {
    require Relianoid::API40::System::Log;
...
if ($q->path_info =~ qr{^/system/backup}) {
    require Relianoid::API40::System::Backup;
