...
Relianoid::VPN::SiteToSite::Core

=cut

=pod

=head1 getVpnSiteToSiteExists

	Check if Site-to-Site VPN exits.
...
sub getVpnSiteToSiteExists ($vpn_name) {
    my $rc    = 1;
    my $found = 0;
...
=head1 getVpnSiteToSiteStatus

	Gets the status of a Site-to-Site VPN connection.

Parameters:
	vpn_name - vpn connection name.

Returns:
	String - Site-to-Site VPN status . empty|connecting|up|down

=cut

sub getVpnSiteToSiteStatus ($vpn_name) {
    require Relianoid::VPN::Core;
    ...
    if ($status eq $vpn_config->{STATUS_UP}) {
        if (&getVpnSiteToSiteRouteStatus($vpn_name) eq $vpn_config->{STATUS_DOWN}) {
...
=head1 getVpnSiteToSiteRouteStatus

	Gets the status of the routing Site-to-Site VPN connection.

Parameters:
	vpn_name - vpn connection name.

Returns:
	String - Site-to-Site VPN Routing status . up|down

=cut

sub getVpnSiteToSiteRouteStatus ($vpn_name) {
    require Relianoid::VPN::Core;

    if (&getVpnRestartStatus($vpn_name) eq "true") {
        return &getVpnSiteToSiteRouteSystemStatus($vpn_name);
    }
    else {
        return &getVpnSiteToSiteRouteConfigStatus($vpn_name);
    }
}

=pod

=head1 getVpnSiteToSiteRouteConfigStatus

	Gets the status of the routing Site-to-Site VPN connection using config values.
...
sub getVpnSiteToSiteRouteConfigStatus ($vpn_name) {
    require Relianoid::VPN::Core;
...
=head1 getVpnSiteToSiteRouteSystemStatus

	Gets the status of the routing Site-to-Site VPN connection using system values.
...
sub getVpnSiteToSiteRouteSystemStatus ($vpn_name) {
    require Relianoid::VPN::Core;
    my $vpn_config = &getVpnModuleConfig();
    my $status     = $vpn_config->{STATUS_DOWN};
