...
"Invalid backend connection limit value, accepted values are 0 or higher.";
        ...
        # Create backend
        my $status =
          &setL4FarmServer($farmname, $id, $json_obj->{ip}, $json_obj->{port},
            $json_obj->{weight}, $json_obj->{priority}, $json_obj->{max_conns},
          );
...
sub new_service_backend    # ( $json_obj, $farmname, $service )
{
    ...
    if ($type eq "gslb" && $eload) {
        &eload(
            module => 'Relianoid::API31::Farm::GSLB',
...
"It's not possible to modify the backend with ip $json_obj->{ip}.";
        ...
        my $status =
          &setDatalinkFarmServer($id_server, $be->{ip}, $be->{interface},
            $be->{weight}, $be->{priority}, $farmname);
...
sub modify_service_backends    #( $json_obj, $farmname, $service, $id_server )
{
    ...
    my $status =
      &setHTTPFarmServer($id_server, $be->{ip}, $be->{port}, $be->{weight},
        $be->{timeout}, $farmname, $service);
