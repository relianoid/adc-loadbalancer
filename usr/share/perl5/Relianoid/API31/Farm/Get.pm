...
sub farms_name_summary    # ( $farmname )
{
    ...
    if ($type =~ /https?/) {
        require Relianoid::API31::Farm::Get::HTTP;
...
sub farms_name    # ( $farmname )
{
    ...
    if ($type eq 'l4xnat') {
        require Relianoid::API31::Farm::Get::L4xNAT;
        &farms_name_l4($farmname);
    }
    if ($type eq 'datalink') {
        require Relianoid::API31::Farm::Get::Datalink;
        &farms_name_datalink($farmname);
    }
    if ($type eq 'gslb' && $eload) {
        &eload(
            module => 'Relianoid::API31::Farm::Get::GSLB',
