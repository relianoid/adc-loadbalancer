...
sub modify_farm    # ( $json_obj, $farmname )
{
    ...
    if ($type eq "http" || $type eq "https") {
        require Relianoid::API31::Farm::Put::HTTP;
        &modify_http_farm($json_obj, $farmname);
    }

    if ($type eq "l4xnat") {
        require Relianoid::API31::Farm::Put::L4xNAT;
        &modify_l4xnat_farm($json_obj, $farmname);
    }

    if ($type eq "datalink") {
        require Relianoid::API31::Farm::Put::Datalink;
        &modify_datalink_farm($json_obj, $farmname);
    }

    if ($type eq "gslb" && $eload) {
        &eload(
            module => 'Relianoid::API31::Farm::Put::GSLB',
