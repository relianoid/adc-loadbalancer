...
if ($q->path_info =~ qr{^/certificates}) {
    require Relianoid::API31::Certificate;
...
if ($q->path_info =~ qr{^/farms/$farm_re/certificates}) {
    require Relianoid::API31::Certificate::Farm;
...
if ($q->path_info =~ qr{^/farms/$farm_re(?:/services/$service_re)?/backends}) {
    require Relianoid::API31::Farm::Backend;
...
if ($q->path_info =~ qr{^/farms/$farm_re/services}) {
    require Relianoid::API31::Farm::Service;
...
if ($q->path_info =~ qr{^/farms}) {
    if ($ENV{REQUEST_METHOD} eq 'GET') {
        require Relianoid::API31::Farm::Get;
    ...
    if ($ENV{REQUEST_METHOD} eq 'POST') {
        require Relianoid::API31::Farm::Post;
    ...
    if ($ENV{REQUEST_METHOD} eq 'PUT') {
        require Relianoid::API31::Farm::Put;
    ...
    if ($ENV{REQUEST_METHOD} eq 'DELETE') {
        require Relianoid::API31::Farm::Delete;
...
if ($q->path_info =~ qr{^/interfaces/nic}) {
    require Relianoid::API31::Interface::NIC;
...
if ($q->path_info =~ qr{^/interfaces/vlan}) {
    require Relianoid::API31::Interface::VLAN;
...
if ($q->path_info =~ qr{^/interfaces/virtual}) {
    require Relianoid::API31::Interface::Virtual;
...
if ($q->path_info =~ qr{^/interfaces/gateway}) {
    require Relianoid::API31::Interface::Gateway;
...
if ($q->path_info =~ qr{^/interfaces$}) {
    require Relianoid::API31::Interface::Generic;
...
if ($q->path_info =~ qr{^/graphs}) {
    ...
    GET qr{^/graphs/farms/($farm_re)/($frequency_re)$} =>
      \&get_frec_farm_graphs;
}

# System
if ($q->path_info =~ qr{^/system/dns}) {
    require Relianoid::API31::System::Service::DNS;
...
if ($q->path_info =~ qr{^/system/snmp}) {
    require Relianoid::API31::System::Service::SNMP;
...
if ($q->path_info =~ qr{^/system/ntp}) {
    require Relianoid::API31::System::Service::NTP;
...
if ($q->path_info =~ qr{^/system/users}) {
    require Relianoid::API31::System::User;
...
if ($q->path_info =~ qr{^/system/log}) {
    require Relianoid::API31::System::Log;
...
if ($q->path_info =~ qr{^/system/backup}) {
    require Relianoid::API31::System::Backup;
...
if ($q->path_info =~ qr{^/system/(?:version|license|supportsave)}) {
    require Relianoid::API31::System::Info;
