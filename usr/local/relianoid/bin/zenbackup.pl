...
my $exclude_52_60 =
"--exclude=hostname --exclude=zlbcertfile.pem --exclude=/etc/cron.d/relianoid --exclude=global.conf --exclude=ssh_brute_force.txt --exclude=cluster.conf --exclude=relianoid_version";

if ($action eq "-c") {
    my $backupfor      = &getGlobalConfiguration('backupfor');
    my $version        = &getGlobalConfiguration('version');
    my $z_version_file = '/relianoid_version';
...
"Will be kept: current hostname, global.conf and activation certificate file.\n";
    ...
    my @eject =
      `$tar $exclude_52_60 -xvzf $backupdir\/backup-$name.tar.gz -C /`;
    print "@eject\n";
    ...
    my @old_config =
`tar -axf  $backupdir\/backup-$name.tar.gz usr/local/relianoid/config/global.conf -O`;
    ...
    foreach my $config_line (@old_config) {
        ...
        if ($config_line =~ /^\$ntp = \"(.*)\";$/) {
            ...
            if ($ntp_old ne $ntp_new) {
                print
"Warning! old global.conf ntp value : $ntp_old , new value : $ntp_new\n";
            }
        }
